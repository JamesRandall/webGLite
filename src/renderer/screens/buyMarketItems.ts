...
export function createBuyMarketItemsRenderer(draw2d: Primitives) {
    return function renderMarketPlace(game: Game) {
        ...
        if ((mouseCharacterPosition[1] > y+1) && (mouseCharacterPosition[1] < y + 2 + game.marketItems.length)) {
            ...
            if (game.player.controlState.mouseDown && !game.player.previousControlState.mouseDown) {
                ...
                if (mouseCharacterPosition[0] <= forSaleEnd) {
                    ...
                    const units = Math.min(game.player.controlState.shiftPressed ? 5 : 1, game.player.cargoHoldContents[itemIndex])
                    if (units > 0) {
                        item.quantityForSale += units
                        game.player.cargoHoldContents[itemIndex] -= units
                        game.player.cash += units * item.unitPrice
        ...
        game.marketItems.map((item,index) => ({item,index})).forEach(({item,index}) => {
            draw2d.text.draw(item.name, [1,y])
            draw2d.text.draw(item.unit, [14,y])
            draw2d.text.draw(item.unitPrice.toLocaleString(undefined,{maximumFractionDigits: 1, minimumFractionDigits: 1 }), [17,y])
        ...
        draw2d.text.draw(`Cash: ${game.player.cash.toLocaleString(undefined,{maximumFractionDigits: 1, minimumFractionDigits: 1 })} Cr`, [1,y+1])
