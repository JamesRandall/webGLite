...
// For rear view: stars start near center (low Z) and move outward
function createRandomStarForRearView() {
  // Start near center with small random offset
  const angle = Math.random() * Math.PI * 2
  const radius = Math.random() * 0.15 + 0.05 // small radius near center
  return vec3.fromValues(Math.cos(angle) * radius, Math.sin(angle) * radius, Math.random() * 0.3)
}
...
export function updateStardust(game: Game, timeDelta: number) {
  ...
  game.localBubble.stardust = game.localBubble.stardust.map((sd) => {
    ...
    if (isRearView) {
      // Rear view: stars move away from camera (Z increases), start near center
      const speed = (z * distancePerSecondAtBack * 10 + distancePerSecondAtBack) * timeDelta * playerSpeed
      let newZ = sd[2] + speed
      if (newZ >= 1.0) {
        return createRandomStarForRearView()
      }
      ...
      if (
        ...
        // Reverse roll direction for rear view, pitch stays same
      ...
      // Front view: stars move toward camera (Z decreases), X/Y spread outward
