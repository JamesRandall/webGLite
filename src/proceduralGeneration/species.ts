...
export function getSpecies(seed: ProceduralSeed) {
    if (!seed.s2_lo.isBitSet(7)) { return 'Human Colonials' }
    const components:string[] = []
    const sizeIndex = seed.s2_hi.getBitRange(2,3)
    if (sizeIndex < size.length) { components.push(size[sizeIndex]) }
    const adjectiveIndex = seed.s2_hi.getBitRange(5,3)
    if (adjectiveIndex < adjective.length) { components.push(adjective[adjectiveIndex])}
    const styleIndex = (seed.s0_hi ^ seed.s1_hi).getBitRange(0,3)
    if (styleIndex < style.length) { components.push(style[styleIndex])}
