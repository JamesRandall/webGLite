...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    for(int i = 0; i < nsamples; i++)
    {
        ...
        color += texture(uSampler, uv * scale + center);
    }
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    vec2 blur_center = iResolution.xy/2.0;
    vec2 pix_l = 1.0/iResolution.xy;

    vec2 uv = fragCoord * pix_l;
    vec2 d = fragCoord-blur_center.xy;
    vec2 dir = normalize(d);

    vec3 color = fast_gaussian_blur(uSampler,uv,iResolution.xy,dir,BLUR*0.01*length(d)).xyz;
